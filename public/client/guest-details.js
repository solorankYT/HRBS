  ...
  booking.rooms.forEach((room, index) => {
    ...
    div.innerHTML = `
      <div class="room-header">
        <div>
          <div class="room-name fw-semibold">${room.type}</div>
          <div class="room-dates small text-secondary mt-1">
            ${booking.check_in} - ${booking.check_out} • ${nights} night${nights > 1 ? 's' : ''}
          </div>
        </div>
        <div class="text-end">
          <div>${room.price.toLocaleString()} <span class="small text-muted">/ night</span></div>
          <div class="fw-bold mt-1">₱${roomTotal.toLocaleString()}</div>
        </div>
      </div>
    `;
...
function renderGuestAccordion() {
  ...
  booking.rooms.forEach((room, i) => {
    ...
    div.innerHTML = `
      ...
      <div id="collapse${i}" class="accordion-collapse collapse ${i === 0 ? 'show' : ''}" data-bs-parent="#roomsAccordion">
        <div class="accordion-body">
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control guest-requests" rows="2" placeholder="Any special requests (optional)"></textarea>
          </div>
        </div>
      </div>
    `;
...
function confirmBooking() {
  // collect guest info
  const names = Array.from(document.querySelectorAll('.guest-name')).map(i => i.value.trim());
  const emails = Array.from(document.querySelectorAll('.guest-email')).map(i => i.value.trim());
  const phones = Array.from(document.querySelectorAll('.guest-phone')).map(i => i.value.trim());
  const requests = Array.from(document.querySelectorAll('.guest-requests')).map(i => i.value.trim());

  if (names.some(n => !n) || emails.some(e => !e) || phones.some(p => !p)) {
  ...
  if (!document.getElementById('privacyTerms').checked || !document.getElementById('bookingConditions').checked) {
    return alert('Please agree to all terms.');
  }

  const payment = document.querySelector('.payment-option.selected');
  ...
  const bookingData = {
    ...
    rooms: booking.rooms,
    guest_info: booking.rooms.map((r, i) => ({
      name: names[i],
      email: emails[i],
      phone: phones[i],
      requests: requests[i]
    })),
    payment: payment.dataset.method
  };

  console.log('Booking ready to submit:', bookingData);
  alert('Booking confirmed! Check console for details.');
}

// --- Load booking from URL params ---
function loadBooking() {
  const params = new URLSearchParams(window.location.search);
  booking.check_in = params.get('check_in');
  booking.check_out = params.get('check_out');
  booking.number_of_guests = params.get('number_of_guests');
...
async function getAvailableRooms(checkIn, checkOut, guests) {
  const params = new URLSearchParams({ check_in: checkIn, check_out: checkOut, number_of_guests: guests });

  const res = await fetch(`/api/guest/rooms/availability?${params.toString()}`, {
...
async function addRoomToBooking(roomId) {
  try {
    ...
    const alreadyAdded = booking.rooms.filter(r => r.id === roomId).length;
    if (alreadyAdded >= selectedRoom.available_count) {
      return alert(`Sorry, no more ${selectedRoom.type} available for these dates.`);
    }

    booking.rooms.push(selectedRoom);
    renderCart();
    renderGuestAccordion();
  } catch (error) {
    console.error(error);
...
//       return alert(`Sorry! ${selectedRoom.type} is no longer available.`);
//     }
...
//     bootstrap.Modal.getInstance(document.getElementById('roomSelectModal')).hide();
//   }
// });
